-- Функция для загрузки всех 300+ продуктов из каталога для пользователя
-- Эта функция создает продукты для всех товаров в каталоге
...
BEGIN
    ...
    SELECT COUNT(*) INTO catalog_products_count FROM public.product_catalog;
...
-- Функция для загрузки только популярных продуктов (20-30 штук)
